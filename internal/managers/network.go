...
// the USB dongle interface. wpa_supplicant is disabled at build time to avoid
// conflicting with hostapd on wlan0, and only starts automatically when networkd
// processes a netplan wifis: section. The fix writes netplan first (which includes
...
func (m *NetworkManager) setOnlineWiFiMode(ctx context.Context, ssid, password string, staticIP models.StaticIPConfig) error {
	...
	// B126: Write netplan FIRST. The generated YAML includes the access-points
	// block with WiFi credentials. When networkd processes the wifis: section it
	// automatically starts wpa_supplicant, associates to the AP, and runs DHCP
	// (or applies static IP). This replaces the old hal.ConnectWiFi (wpa_cli)
	// call that failed because wpa_supplicant wasn't running yet.
	...
	// Poll for IP — networkd starts wpa_supplicant, associates, then DHCP/static.
	// WiFi association + DHCP can be slow, use 30s timeout.
	if err := m.pollForIP(ctx, iface, 30*time.Second); err != nil {
		log.Warn().Err(err).Str("iface", iface).
			Msg("NetworkManager: WiFi connection timeout — check credentials or signal")
		// Don't return error — netplan is written for reboot persistence,
		// and the user can see the status in the dashboard.
	}
